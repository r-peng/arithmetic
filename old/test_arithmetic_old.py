...
a = np.random.rand()
f = [x.reshape(1,d,1)]
f = amt._compose0(f,a,'+',thresh,bdim,max_iter)
...
f = [x1.reshape(1,d,1)]
g = [x2.reshape(1,d,1)]
h = amt._compose1(f,g,'+',thresh,bdim,max_iter)
...
f2 = [x2.reshape(1,d,1)]
f3 = [x3.reshape(1,d,1)]
f = amt._compose1(f1,f2,'+',thresh,bdim,max_iter)
g = amt._compose1(f1,f3,'+',thresh,bdim,max_iter)
h = amt._compose2(f,g,1,'+',thresh,bdim,max_iter)
...
h2 = amt._compose2(h,h,3,'+',thresh,bdim,max_iter)
out = amt._contract(h2,[i2,i1,i3])
print('+',abs(out[0,0]-2*(x1[0]+x2[1])*(x1[0]+x3[2])),amt._get_bdim(h2))
hsq = amt._compose2(h,h,3,'*',thresh,bdim,max_iter)
...
for i in range(1,n):
    tn = amt._compose1(tn,[x[i,:].reshape(1,d,1)],'+',thresh,bdim,max_iter)
